...
#ifdef IBV
    JTRACE ( "BAB: ConnectionToFds::create fd value:" ) ( fds[i] );
...
{
  JTRACE ( "BAB: retrieveP fd value:" ) ( fd );
...
{
  JTRACE ( "BAB: KernelDeviceToConnection::create fd value:" ) ( fd );
  dmtcp::string device = fdToDevice ( fd );
...
dmtcp::string dmtcp::KernelDeviceToConnection::fdToDevice ( int fd, bool noOnDemandConnection )
{
  ...
  JTRACE ( "BAB: fd value:" ) ( fd );
  JTRACE ( "BAB: _procFDPath value:" ) ( fd );
  if ( isBadFd )
  {
...
void dmtcp::KernelDeviceToConnection::prepareForFork ( )
{
  ...
  for ( size_t i=0; i<fds.size(); ++i )
  {
    if ( _isBadFd ( fds[i] ) ) continue;
    if ( ProtectedFDs::isProtected ( fds[i] ) ) continue;
    JTRACE ( "BAB: prepareForFork fd value:" ) ( fds[i] );
