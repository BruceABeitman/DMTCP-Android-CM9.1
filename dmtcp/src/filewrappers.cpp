...
#endif
{
  ...
  int s_time = 1000.0 * now.tv_sec + (double) now.tv_nsec / 1e6;
  JTRACE ( "BAB: filewrappers: system time:" ) ( s_time );
  JTRACE ( "BAB: filewrappers:ioctlwrapper d value:" ) ( d );
  ...
  if (send_sigwinch && request == TIOCGWINSZ) {
    ...
    dmtcp::Connection *con =
      dmtcp::KernelDeviceToConnection::instance().retrieveP(d);
    if (con) {
