  ...
  dmtcp::DmtcpCoordinatorAPI::createNewConnectionToCoordinator (bool dieOnError)
{
  ...
  if (!fd.isValid() && !dieOnError) {
    JTRACE("fd for Coordinator is NOT Valid! fd: ")(fd.isValid());
    JTRACE("coordinatorAddr, coordinatorPort ")(coordinatorAddr)(coordinatorPort);
    return fd;
  }
...
bool dmtcp::DmtcpCoordinatorAPI::connectToCoordinator(bool dieOnError /*= true*/)
{
  ...
  if (!_coordinatorSocket.isValid() && !dieOnError) {
    JTRACE("BAB Unable to connect to coordinator!!");
    return false;
  }
  ...
  JTRACE("BAB Able to connect to coordinator!!");
  return true;
}
  ...
  DmtcpMessageType msgType /*= DMT_HELLO_COORDINATOR*/)
{
  ...
  JTRACE("BAB created hello_local");
  JTRACE("BAB checking ENV_VAR_VIRTUAL_PID")(ENV_VAR_VIRTUAL_PID);

  if (getenv(ENV_VAR_VIRTUAL_PID) == NULL) { 
    JTRACE("BAB ENV_VAR_VIRTUAL_PID is null");
    hello_local.virtualPid = -1;
  } else {
    JTRACE("BAB ENV_VAR_VIRTUAL_PID is NOT null, set virtualPID to value");
...
/*
  if ( interval != NULL ) {
    JTRACE("BAB interval is NOT null");
...
*/
  ...
  if (prefixPathEnv != NULL) {
    JTRACE("BAB prefixPathEnv is NOT null");
    ...
    if (utilDirPrefix == jalib::Filesystem::ResolveSymlink(prefixPathEnv)) {
      JTRACE("BAB prefixDir = prefixPathEnv");
      prefixDir = prefixPathEnv;
    } else {
      JTRACE("BAB prefixDir = utilDirPrefix");
...
{
  JTRACE("sendUserCommand c: result:")(c)(result);
  ...
  // Android didn't let me compile with reply or reply.assertValid?
  //JTRACE("Coordinator reply: ")(reply.assertValid());
    ...
    DmtcpCoordinatorAPI coordinatorAPI;
    {
      if ( coordinatorAPI.tryConnectToCoordinator() == false ) {
        ...
        _real_exit(DMTCP_FAIL_RC);
         ...
         JTRACE("BAB Coordinator found.  No need to start a new one.");
      }
    }
