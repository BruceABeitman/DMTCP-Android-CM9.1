                                               ...
                                               DmtcpMessageType type )
{
  ...
  do {
    ...
    JTRACE("BAB in waitForCoordinatorMsg do-while msg.type: \n") (msg.type);
...
void dmtcp::DmtcpWorker::startSynchronize()
{
  ...
  //JTRACE("BAB called startSynchronize, MUTEX - syncLock: ") (syncLock);
  int sL = pthread_mutex_lock(&syncLock);
  JTRACE("BAB User called startSynchronize, with lock returning: ") (sL);
  //JTRACE("BAB called startSynchronize, MUTEX - syncLock: ") (syncLock);
}
...
void dmtcp::DmtcpWorker::waitSynchronize()
{
  ...
  //JTRACE("BAB called waitSynchronize, MUTEX - syncCond: ") (syncCond);
  //JTRACE("BAB called waitSynchronize, MUTEX - syncLock: ") (syncLock);
  
  int cw = pthread_cond_wait(&syncCond, &syncLock);
  JTRACE("BAB User called waitSynchronize, with cond_wait returning: ") (cw);

  //JTRACE("BAB called waitSynchronize, MUTEX - syncCond: ") (syncCond);
  //JTRACE("BAB called waitSynchronize, MUTEX - syncLock: ") (syncLock);

  JTRACE("Synchronized!\n");
  int mul = pthread_mutex_unlock(&syncLock);
  JTRACE("BAB User called waitSynchronize, with tryUNlock returning: ") (mul);
