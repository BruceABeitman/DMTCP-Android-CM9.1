  ...
  case 'x': case 'X': /* SYNCHRONIZE */
    {
      ...
      JTRACE("BAB SYNCHRONIZE s.numPeers, _sync") (s.numPeers) (_sync);
      if (s.numPeers == _sync) {
        _sync = 0;
        JTRACE("release...!") (order);
        broadcastMessage ( DMT_SYNCHRONIZE );
      //} else {
        // BAB "dumb" fix for multiple app communication issues, bypass check!
      //  _sync = 0;
      //  JTRACE("BAB app fix!! release...!") (order);
      //  broadcastMessage ( DMT_SYNCHRONIZE );
      }
  ...
  else
  {
    ...
    switch ( msg.type )
    {
      case DMT_OK:
      {
        ...
        {
          JTRACE ("BAB oldState == WorkerState::RUNNING && newState == WorkerState::SUSPENDED");
