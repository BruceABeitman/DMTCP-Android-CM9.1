...
jalib::JSocket::JSocket()
{
  ...
  JTRACE("BAB Created socket in JSocket, sockfd: ") (_sockfd);
  JTRACE("BAB Created socket in JSocket, errno: ") (errno);
}
...
bool jalib::JSocket::connect ( const JSockAddr& addr, int port )
{
  ...
  if (addr._addr->sin_port == (unsigned short)-2)
    // This line below, appeared to crash DMTCP checkpointing?!
    //JTRACE("BAB Bad port!!");
    return false;
  ...
  //JTRACE("BAB jalib::JSocket::connect ret: ")(ret);
  return ret;
}
                               ...
                               socklen_t addrlen, int port )
{
  ...
  bool conn_ret = jalib::connect( _sockfd, (sockaddr*)&addrbuf, addrlen ) == 0;
  JTRACE("BAB jalib::JSocket::connect ret: ")(conn_ret);
  return conn_ret;
}
...
{
  JTRACE("BAB bind socket in JSocket, sockfd: ") (_sockfd);
  return jalib::bind ( _sockfd, addr, addrlen ) == 0;
}

bool jalib::JSocket::listen ( int backlog/* = 32*/ )
{
  JTRACE("BAB listen socket in JSocket, sockfd: ") (_sockfd);
...
{
  JTRACE("BAB Reading socket in JSocket, sockfd: ") (len);
  return jalib::read ( _sockfd,buf,len );
}

ssize_t jalib::JSocket::write ( const char* buf, size_t len )
{
  JTRACE("BAB Writing socket in JSocket, sockfd: ") (len);
  // return ::write ( _sockfd,buf,len );
  ssize_t ret = ::write ( _sockfd,buf,len );
  JTRACE("BAB Writing socket in JSocket, sockfd: ") (_sockfd);
  JTRACE("BAB Writing socket in JSocket, ret: ") (ret);
  JTRACE("BAB Writing socket in JSocket, errno: ") (errno);
  return ret;
}

ssize_t jalib::JSocket::readAll ( char* buf, size_t len )
{
  JTRACE("BAB readAll socket in JSocket, sockfd: ") (len);
  int origLen = len;
  while ( len > 0 )
  {
    fd_set rfds;
...
{
  JTRACE("BAB readAll writeAll in JSocket, sockfd: ") (len);
  int origLen = len;
  while (len > 0) {
    fd_set wfds;
    struct timeval tv;
